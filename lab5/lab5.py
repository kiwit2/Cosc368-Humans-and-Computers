...
class fitts(object):
    ...
    def log(self):
        ...
        current_time = 0
    #=====================uses key of time, LAB 5 ==========================
        if self.keyValuePair[(self.a[self.index][0], self.a[self.index][1], self.repdict[pos])] == None:
            self.keyValuePair[(self.a[self.index][0], self.a[self.index][1], self.repdict[pos])] = self.Total
        else:
            #=========finds the mean of all times for that pair-----------
            previous_time = self.keyValuePair[(self.a[self.index][0], self.a[self.index][1], self.repdict[pos])]
            current_time = (previous_time + self.total) / 2
        #need to check if i have to accumulate all the times
        self.file2.write(str(self.a[self.index][0])+ " " + str(self.a[self.index][1]) + 
                                " " + str(self.a[self.index][1])+ " " + str(self.repdict[pos]) + " " 
                                + str(round(current_time, 1)) + "\n" )        
